...
lazy_static! {
    static ref LINKEDIN_KEYS: JwkKeys = serde_json::from_str(LINKEDIN_JWKS).expect("Failed to parse JWK");
}
...
impl IdentityProvider {
    pub fn validate(&self, token: &str) -> Result<PublicOutput, OidcErr> {
        match self {
            Self::LinkedIn => {
                let decoded = decode_token::<LinkedInClaims>(token, &LINKEDIN_KEYS).unwrap();
                Ok(PublicOutput {
...
pub struct LinkedInClaims {
    ...
    pub nonce: String,
    pub email: String,
    pub name: String, // New field for name
    pub email: String, // New field for last name
}
...
pub mod test_oidc_validator {
    ...
    use crate::LINKEDIN_JWKS;
    use std::env;

    use super::{decode_token, LinkedInClaims};

    #[test]
    fn test_validate_linkedin_jwt_valid_token() {
        let jwt = env::var("jwt").expect("jwt not set");
        let decoded = decode_token::<LinkedInClaims>(&jwt, &LINKEDIN_KEYS).unwrap();
